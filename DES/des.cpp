...
enum    {ENCRYPT,DECRYPT};// ENCRYPT:,DECRYPT:
...
int main()
{
    char key[9], str[9];
    fstream file;
    file.open("E:\\Desktop\\key.txt", ios::in);
    file.read(key, 8);
    file.close();
    
    file.open("E:\\Desktop\\mingwen.txt", ios::in);
    file.read(str, 8);
    file.close();

    puts("\n<<<<<<<<<<<<<<<<   Before encrypting   >>>>>>>>>>>>>>>>>>\n");
    cout << "The key is: " << key << endl<<"\nThe mingwen is: " << str << endl;
    Des_SetKey(key);
    Des_Run(str, str, ENCRYPT);
    puts("\n<<<<<<<<<<<<<<<<   After decrypting   >>>>>>>>>>>>>>>>>>>\n");
    Des_Run(str, str, DECRYPT);
    cout<<"\nThe miwen is: "<<str<<endl;
